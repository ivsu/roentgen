...
class Bot:
    def __init__(self, hp, state=None, bot_id=None, shift=None, index=None):
        ...
        Args:
            hp: инстанс гиперпараметров
            state: словарь состояния бота
            bot_id: ID бота, с которым он будет создан
            shift: индекс популяции, к которой относится бот
            index: индекс бота в популяции
        """
